...
chrome.notifications.onClicked.addListener((myNotificationId)=>{
	...
	if(myNotificationId=='5118')window.open('https://www.5118.com/toutiao/5118PlugIn');
	if(myNotificationId=='zhihu')window.open('https://zlb.5118.com/');
   
  });

var myNotificationId = "5118";
function showNotification(listmsg) {
	var opt = {
		type: "basic",
		title: "5118插件更新通知",
		message: listmsg,
		iconUrl: "img/5118_logo.png",
		// 文字列表
		// items: listmsg,
		//按钮功能,设置标题和图片
...
chrome.notifications.onButtonClicked.addListener(function (notifId, btnIdx) {
	if (notifId === myNotificationId) {
		if (btnIdx === 1) {//第一个按钮
			window.open('https://www.5118.com/toutiao/5118PlugIn');
		} else if (btnIdx === 0) {
			SetSwitchValue("ON");
...
chrome.extension.onMessage.addListener(
    function(request, sender, sendResponse) {
		...
		if (request.greeting === "add_all_js"){

			const jqurl="https://aiplugin.5118.com/static/OnlineJs/jquery-3.3.1.js";
			...
			xhr.onreadystatechange = function() { 
				if (xhr.readyState == 4  && xhr.status == 200) { 
					getCurrentTabId(tabId => {
						...
						config_url="https://aiplugin.5118.com/5118_Plugin_Config_New.txt?p="+timesversion;
						let xhr = new XMLHttpRequest(); 
		...
		if(request.greeting === "innerzhihujs"){
			const jqurl="https://aiplugin.5118.com/static/OnlineJs/jquery-3.3.1.js";
			let xhr = new XMLHttpRequest(); 
			xhr.open("GET", jqurl, true); 
			xhr.onreadystatechange = function() { 
				if (xhr.readyState == 4  && xhr.status == 200) { 
					getCurrentTabId(tabId => {
						...
						config_url="https://aiplugin.5118.com/5118_Plugin_Iframe_Config.txt?p="+timesversion;
						let xhr = new XMLHttpRequest(); 
						xhr.open("GET", config_url, true); 
						xhr.onreadystatechange = function() { 
							if (xhr.readyState == 4  && xhr.status == 200) { 
								...
								for(key in configinfo )
								{
									...
									if(type=="js")
									{
										...
										GetOnlineiframeJs(url+"?p="+timesversion);
									}
								}
							} 
						} 
