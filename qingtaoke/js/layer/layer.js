...
!function (window, undefined) {
    ...
    var $, win, ready = {
        ...
        //屏蔽Enter触发弹层  e.preventDefault()
        enter: function (e) {

            if (e.keyCode === 13){
                console.log(e);
                console.log(e.event);
                if(e.target.id!=='editDivContent'){
                    e.preventDefault();
                }
            }
        },
...
//容器
    Class.pt.vessel = function (conType, callback) {
        ...
        callback([
            ...
            //主体
            '<div class="' + doms[0] + ' ' + (doms.anim[config.shift] || '') + (' layui-layer-' + ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin || '') + '" id="' + doms[0] + times + '" type="' + ready.type[config.type] + '" boxtype="' + config.boxtype + '" times="' + times + '" showtime="' + config.time + '" conType="' + (conType ? 'object' : 'string') + '" style="z-index: ' + zIndex + '; width:' + config.area[0] + ';height:' + config.area[1] + (config.fix ? '' : ';position:absolute;') + '">'
...
//主入口
    ...
    'function' === typeof define ? define('layer', ['jquery'], function ($) {
        ready.run();
        return layer;
    }) : function () {
        window.layer = layer;
